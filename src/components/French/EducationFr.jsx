

function EducationFr() {
    return (
        <div id="Education">
            <span className="overline">
                <h2>Education</h2></span>
            <ul className="education-list">

                <li className="year encours">En cours</li>
                <div className="couseBlock">
                    <li className="course">Developer Web & Web Mobile</li>
                    <li>Wild Code School • Valence</li>
                </div>
                <li className="year afpE">2022 (oct-nov)</li>
                <div className="couseBlock">
                    <li className="course">Apple Foundation Extended Program</li>
                    <li>Simplon Lille</li></div>
                <li className="year afp">2022 (sept)</li>
                <div className="couseBlock">
                    <li className="course">Apple Foundation Program</li>
                    <li>Simplon Lyon</li></div>
                <li className="year master">2017-2018</li>
                <div className="couseBlock">
                    <li className="course">1ere Annee Master Etudes Anglophones</li>
                    <li >Universite Lumière Lyon II</li></div>
                <li className="year llce">2017</li>
                <div className="couseBlock">
                    <li className="course">Diplome Licence LLCE Anglais</li>
                    <li>Universite Grenoble Alpes</li></div>
                <li className="year capa">2010</li>
                <div className="couseBlock">
                    <li className="course">Diplome Capacite en Droit</li>
                    <li> Universite Lumière Lyon II</li></div>
            </ul>
        </div>
    );
}

export default EducationFr;
